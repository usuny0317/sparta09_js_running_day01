...
3.	원래 의도대로라면 유저 A와 유저 B 장바구니가 독립적으로 동작해야 하는데, 그렇게 하려면 코드를 어떻게 수정해야 할까요?
    const userBCart = userACart 해당 부분의 복사를 1. 직접 지정하거나 2. A차트의 값을 이용해서 복사하는 것으로 해결할 수 있습니다.
    1. 
    const userBCart = {
        items: [
            { name: "키보드", price: 30000 },
    ...
    const userBCart = {
        items: [
            { name: userACart.item[0].name, price: userACart.item[0].price },
            { name: userACart.item[1].name, price: userACart.item[1].price },
            ],
        }
*/